...
class ExcelService {
  ...
  Future<String?> generateReport({
    ...
    try {
      ...
      _addCell(sheet, currentRow, 2, 'Joined At', bold: true);
      _addCell(sheet, currentRow, 3, 'Connection (min)', bold: true);
      _addCell(sheet, currentRow, 4, 'Verified', bold: true);
      currentRow++;
      ...
      for (final record in currentSessionRecords) {
        ...
        _addCell(sheet, currentRow, 4,
            record.isVerified && record.isPinVerified ? 'Yes' : 'No');
        currentRow++;
      }
      ...
      _addCell(sheet, currentRow, 0, 'Matricule', bold: true);
      _addCell(sheet, currentRow, 1, 'Name', bold: true);
      _addCell(sheet, currentRow, 2, 'Previous Total', bold: true);
      _addCell(sheet, currentRow, 3, 'New Total', bold: true);
      _addCell(sheet, currentRow, 4, 'Percentage', bold: true);
      currentRow++;
      ...
      for (final record in currentSessionRecords) {
        ...
        int newTotal;
        if (record.isVerified && record.isPinVerified) {
      ...
      for (final student in sortedMaster) {
        ...
        _addCell(sheet, currentRow, 2, previousTotal.toString());
        _addCell(sheet, currentRow, 3, student.totalPresence.toString());
        _addCell(sheet, currentRow, 4, '$percentage%');
        currentRow++;
      }
