...
class SessionService {
  ...
  /// Regenerate PIN for the active session
  ...
  /// End a session and stop connection tracking
  ...
  Future<Map<String, dynamic>> getSessionStats(String sessionId) async {
    final records = await _storage.getAttendanceRecords(sessionId);
    final verified = records.where((r) => r.isVerified && r.isPinVerified).length;
    final pending = records.where((r) => !r.isVerified || !r.isPinVerified).length;
